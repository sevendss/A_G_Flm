    ...
    .provider('w5cValidator', [function () {
        ...
        var validatorFn = function () {
            ...
            this.defaultShowError = function (elem, errorMessages) {
                var $elem = angular.element(elem).parent(),
                ...
                var $next = $elem.next();
                if (!$next || !$next.hasClass("w5c-error") || $next.html() != errorMessages[0]) {
                    if ($next.hasClass("w5c-error")) {
                        $next.remove();
                    };
...
(function () {
    angular.module("w5c.validator")
        .directive("w5cFormValidate", ['$parse', 'w5cValidator', '$timeout', '$$verify', '$$modal', '$$base', function ($parse, w5cValidator, $timeout, $$verify, $$modal, $$base) {
            return {
                ...
                link      : function (scope, form, attr, ctrls) {
                            ...
                            if (elemName) {
                                ...
                                if (formCtrl[elemName] && elem && w5cValidator.elemTypes.toString().indexOf(elem.type) > -1 && !w5cValidator.isEmpty(elem.name)) {
                                    if (formCtrl[elemName].$valid) {
                                        if (angular.element(elem).parent().next().hasClass('w5c-error')) {
                                            angular.element(elem).parent().next().remove();
                                        };
                                        continue;
                                    } else {
                    ...
                    if (attr.w5cSubmit && angular.isFunction(formSubmitFn)) {
                        ...
                        form.bind("submit", function (event) {
                            doValidate();
                            if (formCtrl.$errors.length ==0 && angular.isFunction(formSubmitFn)) {
                                // 特殊校验
                                if (angular.isFunction(eval('$$verify.' + formElem.name))) {

                                    var vr = eval('$$verify.' + formElem.name + "(scope)");

                                    if (vr.status === -1) {

                                        $$modal.alert($$base.i18n('common-verifyError'), vr.message, 'error');
